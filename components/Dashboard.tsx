...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      {/* Greeting */}
      <div>
        <p className="text-base font-semibold text-gray-900">{greeting}, {profileName} 👋</p>
        <p className="text-sm text-gray-400">{storeName} · {new Date().toLocaleDateString('en-PH', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="border border-gray-200 rounded-xl p-4 bg-white">
            ...
            <p className="text-2xl font-semibold text-green-600">₱{stats.totalProfit.toLocaleString()}</p>
          </div>
          <div className="border border-gray-200 rounded-xl p-4 bg-white">
            <p className="text-xs text-gray-500 mb-1">Transactions</p>
            <p className="text-2xl font-semibold text-gray-900">{stats.txCount}</p>
          </div>
          <div className="border border-gray-200 rounded-xl p-4 bg-white">
            <p className="text-xs text-gray-500 mb-1">Unpaid Debts</p>
            <p className="text-2xl font-semibold text-gray-900">₱{totalDebt.toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      {insights.length > 0 && (
        <div className="space-y-2">
          ...
          {insights.map((msg, i) => (
            <div key={i} className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-sm text-yellow-800">
              ⚠ {msg}
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        {(products ?? []).length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-8">No products yet. Add one in Inventory.</p>
        ) : (
            ...
            {(products ?? []).slice(0, 8).map((p) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500">₱{p.price} · {p.stock} {p.unit}</p>
                </div>
