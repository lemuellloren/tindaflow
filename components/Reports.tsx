    ...
    filteredSales.forEach((sale) => {
      sale.items.forEach((item) => {
        if (!productTotals[item.productName]) {
          productTotals[item.productName] = { name: item.productName, qty: 0, revenue: 0 };
        }
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[
            ...
            { label: 'Avg. Sale', value: `₱${Math.round(stats.avgSale).toLocaleString()}` },
            { label: 'Margin', value: `${stats.margin.toFixed(1)}%` },
            { label: 'Low Stock Items', value: `${lowStockCount + outCount}`, warn: outCount > 0 },
          ].map((card) => (
            <div
              key={card.label}
            ...
            >
              ...
              <p
                className={`text-xl font-semibold ${
                  card.highlight ? 'text-green-600' : card.warn ? 'text-yellow-600' : 'text-gray-900'
                }`}
              >
                {card.value}
              </p>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{s.txCount} transaction{s.txCount !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                <p className="text-sm font-semibold text-gray-900">₱{s.revenue.toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 text-xs font-semibold shrink-0 mt-0.5">
